...
func init() {
	android.RegisterModuleType("rebellion_generator", GeneratorFactory)
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = rebellionExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = rebellionExpandVariables(ctx, dep_file)
	...
	cmd := rebellionExpandVariables(ctx, String(g.properties.Cmd))
